...
//  int a=0;
  ...
  bIsCheckBoxLooseStartTimeChecked=false;
  bISCheckBoxTightenedStartTimeChecked=false;
  bIsCheckBoxTightenedEndTimeChecked=false;
  bIsCheckBoxLooseEndTimeChecked=false;
}
...
//  delete colorImage;
  ...
  {
      bIsOpenFile=false;
      eventInfo.saveEventInfo("eventInfo.xml");
      oniCapture.Close();
...
{
  ui->horizontalSlider_playControl->setSliderPosition(nNoOfFrameForFile);
  ui->label_frameNoShow->setText(QString::number(nNoOfFrameForFile));
  ...
  //add the show of four label about start time and end time in there
  if(bIsOpenFile&&bIsFilePicInit)
  {
      if(!bIsCheckBoxLooseStartTimeChecked)
      {
        ui->label_looseStartTime->setText(QString::number(oniCapture.GetFrameIndex()));
      }
      if(!bISCheckBoxTightenedStartTimeChecked)
      {
        ui->label_tightenedStartTime->setText(QString::number(oniCapture.GetFrameIndex()));
      }
      if(!bIsCheckBoxTightenedEndTimeChecked)
      {
        ui->label_tightenedEndTime->setText(QString::number(oniCapture.GetFrameIndex()));
      }
      if(!bIsCheckBoxLooseEndTimeChecked)
      {
        ui->label_looseEndTime->setText(QString::number(oniCapture.GetFrameIndex()));
      }
  }
}
...
//      ui->horizontalSlider_playControl->setRange(0,oniCapture.GetNumberOfColorFrames());
      ...
      eventInfo.setTotalFrame(nNumOfFrameForFile);
      eventInfo.clearInfo();
...
void ONIViewDialog::on_checkBox_looseStartTime_clicked()
{
    if(!bIsCheckBoxLooseStartTimeChecked)
    {
        bIsCheckBoxLooseStartTimeChecked=true;
    }
    else
    {
        bIsCheckBoxLooseStartTimeChecked=false;
...
void ONIViewDialog::on_checkBox_tightenedStartTime_clicked()
{
    if(!bISCheckBoxTightenedStartTimeChecked)
    {
        bISCheckBoxTightenedStartTimeChecked=true;
    }
    else
    {
        bISCheckBoxTightenedStartTimeChecked=false;
...
void ONIViewDialog::on_checkBox_tightenedEndTime_clicked()
{
    if(!bIsCheckBoxTightenedEndTimeChecked)
    {
        bIsCheckBoxTightenedEndTimeChecked=true;
    }
    else
    {
        bIsCheckBoxTightenedEndTimeChecked=false;
...
void ONIViewDialog::on_checkBox_looseEndTime_clicked()
{
    if(!bIsCheckBoxLooseEndTimeChecked)
    {
        bIsCheckBoxLooseEndTimeChecked=true;
    }
    else
    {
        bIsCheckBoxLooseEndTimeChecked=false;
...
void ONIViewDialog::on_pushButton_submit_clicked()
{
    //QMessageBox::information(NULL, QString("Info"), ui->label_looseStartTime->text());
    //QMessageBox::information(NULL, QString("Info"), ui->label_tightenedStartTime->text());
    //QMessageBox::information(NULL, QString("Info"), ui->label_tightenedEndTime->text());
    //QMessageBox::information(NULL, QString("Info"), ui->label_looseEndTime->text());
    if( bIsCheckBoxLooseStartTimeChecked &&  bISCheckBoxTightenedStartTimeChecked && bIsCheckBoxTightenedEndTimeChecked  && bIsCheckBoxLooseEndTimeChecked )
    {
        if(bIsOpenFile)
        {
            if(!bIsPausedForFile)
        ...
        bool bIsOK;
        QStringList eventListList;
        eventListList<<"fall"<<"violence";
        QString text = QInputDialog::getItem(NULL, "Input Dialog",
                                                                  "Please input the event type",
                                                                  eventListList,
                                                                  0,
                                                                  false,
                                                                  &bIsOK);
        if(bIsOK)
        {
            eventInfo.addNewEventInfo(text.toStdString(),ui->label_looseStartTime->text().toInt(),ui->label_tightenedStartTime->text().toInt(),ui->label_tightenedEndTime->text().toInt(),ui->label_looseEndTime->text().toInt());
        }
    }

    //reset the checkboxes state
    ui->checkBox_looseStartTime->setChecked(false);
    bIsCheckBoxLooseStartTimeChecked=false;
    ui->checkBox_tightenedStartTime->setChecked(false);
    bISCheckBoxTightenedStartTimeChecked=false;
    ui->checkBox_tightenedEndTime->setChecked(false);
    bIsCheckBoxTightenedEndTimeChecked=false;
    ui->checkBox_looseEndTime->setChecked(false);
    bIsCheckBoxLooseEndTimeChecked=false;
}
